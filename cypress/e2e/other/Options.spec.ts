...
describe('Options', () => {
  ...
  it('Settings', () => {
    ...
    cy.get('[data-testid="nickname-0"] > input').type('Bulbadude');
    cy.get('[data-testid="nickname-0"] > input').should('have.value', 'Bulbadude');
  ...
  it('Report', () => {
    ...
    cy.get('[data-testid="report-type"]').select('Suggestion');
    cy.get('[data-testid="report-type"]').should('have.value', 'suggestion');
    cy.get('[data-testid="report-device"]').select('Mobile');
    cy.get('[data-testid="report-device"]').should('have.value', 'mobile');
    cy.get('[data-testid="report-os"]').select('Android');
    cy.get('[data-testid="report-os"]').should('have.value', 'android');
    cy.get('[data-testid="report-browser"]').select('Firefox');
    cy.get('[data-testid="report-browser"]').should('have.value', 'firefox');
    cy.get('[data-testid="report-selectedgame"]').type('Emerald Kaizo');
    cy.get('[data-testid="report-selectedgame"]').should('have.value', 'Emerald Kaizo');
    cy.get('[data-testid="report-description"]').type('Please implement new feature');
    cy.get('[data-testid="report-description"]').should(
      'have.value',
      'Please implement new feature'
    );
  });
  ...
  it('Custom Status', { scrollBehavior: 'center', viewportWidth: 550 }, () => {
    ...
    cy.get('[data-testid="custom-status-input"] > input').type('Foobar');
    cy.get('[data-testid="custom-status-input"] > input').should('have.value', 'Foobar');
