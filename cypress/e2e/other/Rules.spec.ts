describe('Rules', () => {
  ...
  it('Rule CRUD', () => {
    ...
    cy.get('[data-testid="add-rule"]').click();
    cy.get('[data-testid="add-rule-input"] > input').type('This is a test rule');
    cy.get('[data-testid="add-rule-input"] > input').should('have.value', 'This is a test rule');
    cy.contains('Save').click();
    cy.contains('This is a test rule').should('exist');
    cy.get('[data-testid="edit-rule-3"]').click();
    cy.get('[data-testid="edit-rule-input"] > input').type(' this is now an edited rule');
    cy.get('[data-testid="edit-rule-input"] > input').should(
      'have.value',
      'This is a test rule this is now an edited rule'
    );
    cy.contains('Save').click();
  ...
  it('Custom ruleset', () => {
    ...
    cy.contains('Cancel').click();
    cy.get('[data-testid="add-ruleset"]').click();
    cy.get('[data-testid="add-ruleset-input"] > input').type('Nuzlocke Custom');
    cy.get('[data-testid="add-ruleset-input"] > input').should('have.value', 'Nuzlocke Custom');
    cy.contains('Save').click();
  ...
  context('Smart Rules', () => {
    ...
    it('Smart Rules of level', () => {
      ...
      cy.get('[data-testid="edit-rule-3"]').click();
      cy.get('[data-testid="edit-rule-level-input"] > input').clear();
      cy.get('[data-testid="edit-rule-level-input"] > input').type('2');
      cy.contains('Save').click();
      cy.get('[data-testid="add-rule"]').click();
      cy.contains('Level').click();
  ...
  context('Share text', { browser: 'firefox' }, () => {
    it('Share Error', () => {
      ...
      cy.get('[data-testid="share-textarea"]').should('exist');
      cy.get('[data-testid="share-textarea"]').clear();
      cy.get('[data-testid="share-textarea"]').type('Test');
